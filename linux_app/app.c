...
BOOL TEST_DMA_FIFO(PCIE_HANDLE hPCIe){
	...
	if (bPass){
		bPass = PCIE_DmaFifoWrite(hPCIe, FifoID_Write, pBuff, 104);
		printf("in\n");
		for(i=0;i<104;i++){
			printf("0x%04x\n",*(pBuff+i));}
		if (!bPass)
	...
	if (bPass)
		printf("Fifo Level status mask=%xh\r\n", Status);
	else
		printf("Failed to read Fifo Level status\r\n");
	int intStatus;
	...
	if (bPass){
		...
		bPass = PCIE_DmaFifoRead(hPCIe, FifoID_Read, pBuff, Size);
		printf("out\n");
		for(i=0;i<Size;i++){                                               
                        printf("0x%04x\n",*(pBuff+i));}   
